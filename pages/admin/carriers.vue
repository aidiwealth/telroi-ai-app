  ...
  <div class="ad-page">
    <header class="car-head">
      <div>
        ...
        <p class="car-lede">Voice carrier configuration. Telroi Asterisk runs Nigerian + direct SIP trunks on our own PBX; Other countries route through Twilio / Telnyx. Customers never see any of this.</p>
      </div>
      <button v-if="tab === 'asterisk'" class="btn btn-signal" @click="openNew">+ Add carrier</button>
    </header>

    <nav class="car-tabs">
      <button class="car-tab" :class="{ on: tab === 'asterisk' }" @click="tab = 'asterisk'">Telroi Asterisk</button>
      <button class="car-tab" :class="{ on: tab === 'other' }" @click="tab = 'other'">Other countries</button>
    </nav>

    <section v-show="tab === 'asterisk'" class="car-card">
      ...
      <table v-else class="car-table">
        ...
        <tbody>
          <tr v-for="c in carriers" :key="c.id">
            <td><span class="car-name">{{ c.displayName }}</span><span class="car-slug">{{ c.name }}</span></td>
    ...
    <section v-show="tab === 'other'" class="car-card car-card-pad">
      <p class="car-section-lede">API-based carriers for non-Nigerian regions. Unlike the Asterisk trunks, these use vendor APIs (no PBX config) — they power provisioning and in-browser calling for the US / CA / UK and other Twilio/Telnyx-covered countries.</p>

      <div class="car-vendor">
        <div class="car-vendor-head">
          <span class="car-vendor-name">Twilio</span>
          <span class="car-pill" :class="{ live: cfg.twilioSet }">{{ cfg.twilioSet ? 'Configured' : 'Not set' }}</span>
        </div>
        <div class="car-form-grid">
          ...
          <div class="ad-field"><label>Auth token {{ cfg.twilioSet ? '· set' : '' }}</label><input v-model="twToken" type="password" class="ad-input mono" :placeholder="cfg.twilioSet ? '•••••••• (blank to keep)' : 'auth token'" /></div>
        </div>
      </div>

      <div class="car-vendor">
        <div class="car-vendor-head">
          <span class="car-vendor-name">Telnyx</span>
          <span class="car-pill" :class="{ live: cfg.telnyxSet }">{{ cfg.telnyxSet ? 'Configured' : 'Not set' }}</span>
        </div>
        <div class="car-form-grid">
          <div class="ad-field"><label>API key {{ cfg.telnyxSet ? '· set' : '' }}</label><input v-model="tnKey" type="password" class="ad-input mono" :placeholder="cfg.telnyxSet ? '•••••••• (blank to keep)' : 'KEY…'" /></div>
      ...
      <div class="car-vendor">
        <div class="car-vendor-head">
          <span class="car-vendor-name">Browser voice</span>
          <span class="car-vendor-sub">In-app dialer &amp; Live Call widget (microphone, ringing, audio)</span>
        </div>

        <div class="car-subcard">
          <div class="car-sub-name">Twilio Voice <span class="car-pill" :class="{ live: cfg.twilioVoiceSet }">{{ cfg.twilioVoiceSet ? 'Configured' : 'Not set' }}</span></div>
          <div class="car-form-grid">
        ...
        <div class="car-subcard">
          <div class="car-sub-name">Telnyx WebRTC <span class="car-pill" :class="{ live: cfg.telnyxVoiceSet }">{{ cfg.telnyxVoiceSet ? 'Configured' : 'Not set' }}</span></div>
          <div class="car-form-grid">
      ...
      <div class="car-foot">
        <button class="btn btn-signal" :disabled="savingOther" @click="saveOther">{{ savingOther ? 'Saving…' : 'Save other carriers' }}</button>
        <span v-if="savedOther" class="car-saved">✓ Saved</span>
      </div>
    </section>
    ...
    <div v-if="modalOpen" class="ad-modal-overlay" @click.self="closeModal">
      <div class="ad-modal ad-modal-wide">
        ...
        <div class="car-form-grid">
          ...
          <div class="ad-field"><label>Display name</label><input v-model="form.displayName" class="ad-input" placeholder="Ruach" /></div>
          ...
          <div class="ad-field"><label>SIP domain (optional)</label><input v-model="form.sipDomain" class="ad-input mono" placeholder="blank = use gateway" /></div>
          <div class="ad-field"><label>Auth username</label><input v-model="form.authUser" class="ad-input mono" placeholder="blank for IP-auth" /></div>
          <div class="ad-field"><label>Auth password {{ editingCarrier?.authPassSet ? '· set' : '' }}</label>
          ...
          <div class="ad-field car-span2"><label>Inbound webhook secret {{ editingCarrier?.webhookSecretSet ? '· set' : '' }}</label>
          ...
          <div class="ad-field car-span2 car-toggle-row">
            <label class="car-toggle"><input type="checkbox" v-model="form.enabled" /><span>Enabled — push trunk to PBX</span></label>
        ...
        <div class="ad-modal-foot">
          ...
          <button class="btn btn-signal" :disabled="saving || !canSave" @click="save">{{ saving ? 'Saving…' : (isEdit ? 'Save changes' : 'Create carrier') }}</button>
        </div>
      </div>
    </div>
  </div>
...
const tab = ref<'asterisk' | 'other'>('asterisk');
...
const canSave = computed(() =>
  /^[a-z0-9]{2,32}$/.test(form.name) && form.displayName.trim().length > 0 &&
  /^[0-9]{2,4}$/.test(form.prefix) && form.sipGateway.trim().length > 0
);
...
async function load() {
  ...
  try { carriers.value = await $fetch<Carrier[]>('/api/admin/carriers'); }
  catch (e: any) { loadError.value = e?.data?.message || e?.data?.error?.message || 'Failed to load carriers'; }
...
function openNew() {
  ...
  isEdit.value = false; editingCarrier.value = null; error.value = ''; modalOpen.value = true;
}
function edit(c: Carrier) {
  ...
  isEdit.value = true; editingCarrier.value = c; error.value = ''; modalOpen.value = true;
}
...
async function save() {
  ...
  try {
    ...
    await load(); modalOpen.value = false;
  } catch (e: any) { error.value = e?.data?.message || e?.data?.error?.message || e?.message || 'Save failed'; }
...
async function remove(c: Carrier) {
  ...
  try { await $fetch(`/api/admin/carriers/${encodeURIComponent(c.name)}`, { method: 'DELETE' }); await load(); }
  catch (e: any) { alert(e?.data?.message || e?.data?.error?.message || 'Remove failed'); }
}
...
<style scoped>
.car-head { display: flex; justify-content: space-between; align-items: flex-start; gap: 20px; margin-bottom: 20px; }
.car-title { font-family: var(--font-display); font-size: 28px; }
.car-lede { color: var(--ink-soft); font-size: 13.5px; max-width: 640px; margin-top: 6px; line-height: 1.55; }
.car-tabs { display: flex; gap: 4px; border-bottom: 1px solid var(--rule); margin-bottom: 22px; }
.car-tab { padding: 10px 18px; font-size: 14px; color: var(--ink-soft); border-bottom: 2px solid transparent; margin-bottom: -1px; }
.car-tab:hover { color: var(--ink); }
.car-tab.on { color: var(--signal); border-bottom-color: var(--signal); font-weight: 600; }
.car-card { background: var(--paper); border: 1px solid var(--rule); border-radius: var(--radius-lg); overflow: hidden; }
.car-card-pad { padding: 24px; }
.car-section-lede { font-size: 13px; color: var(--ink-soft); line-height: 1.55; margin-bottom: 22px; max-width: 640px; }
...
.car-vendor { padding: 18px 0; border-bottom: 1px solid var(--rule); }
.car-vendor:first-of-type { padding-top: 0; }
.car-vendor-head { display: flex; align-items: center; gap: 10px; margin-bottom: 14px; flex-wrap: wrap; }
.car-vendor-name { font-weight: 600; font-size: 15px; }
.car-vendor-sub { font-size: 12px; color: var(--ink-mute); }
.car-subcard { background: var(--paper-2); border: 1px solid var(--rule); border-radius: var(--radius); padding: 16px; margin-top: 12px; }
.car-sub-name { font-weight: 600; font-size: 13.5px; margin-bottom: 12px; display: flex; align-items: center; gap: 8px; }
.car-foot { display: flex; align-items: center; gap: 14px; margin-top: 22px; }
.car-saved { color: var(--live); font-size: 13px; font-weight: 500; }
