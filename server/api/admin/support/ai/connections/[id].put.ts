// PUT /api/admin/support/ai/connections/:id -> edit model and/or replace key.
import { z } from 'zod';
...
export default defineEventHandler(async (event) => {
  ...
  const [row] = await db.update(schema.aiConnections).set(patch)
    .where(and(eq(schema.aiConnections.id, id), eq(schema.aiConnections.tenantId, ws.tenantId))).returning();
