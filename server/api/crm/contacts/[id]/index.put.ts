...
const Body = z.object({
  ...
  // Off the board, and back again. Reversible because a contact put away in
  // error should return without ceremony.
...
export default defineEventHandler(async (event) => {
  ...
  // straight through: 'archived' would reach a column that does not exist.
  const { archived, ...rest } = p.data as any;
  const patch: any = { ...rest };
  if (archived !== undefined) patch.archivedAt = archived ? new Date() : null;

  return { contact: await updateContact(s.tenantId, getRouterParam(event, 'id')!, patch) };
});
