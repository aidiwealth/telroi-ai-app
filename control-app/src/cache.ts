...
export async function refreshCache(): Promise<void> {
  try {
    ...
    state = { numbers, blacklist, agentGreetings, loadedAt: Date.now(), ok: true };
    log(`refreshed: ${numbers.size} numbers, ${blacklist.size} blacklist entries, ${agentGreetings.size} agent greetings`);
  } catch (err) {
